...
/*possible to memcpy contents of trGth in a new array that is malloced within the parrallel region and so would be copied for each thread*/
...
#pragma omp parallel private(thread_id, nloops, squareStkA, squares) 
{
	...
	int dzX, dzY; //dip for xline and inline direction - number of traces from origin times the dip per trace
	int samp  =0;
			...
			for (dzX= -maxDip;dzX<=maxDip;dzX++){
				for(dzY = -maxDip;dzY<=maxDip;++dzY){
					...
					//	}// End of loop
						...
						if(sembVal > hold){
							if(sembVal >= thresh){
								dipTrI[halfTr][xNdx][samp] = dzX*rate/reso;
								dipTrX[halfTr][xNdx][samp] = dzY*rate/reso;
								holdDipX = dzX;
								holdDipY = dzY;
							}else{
